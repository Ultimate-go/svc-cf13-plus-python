...
| 指标 | 含义 | 本方案的达成方式 |
|---|---|---|
| **摘要常量** | 客户端保存的东西不能随文件变大 | $\delta = ((U,C), n)$，两个群元素 + 一个整数 |
| **证据常量** | 一份证据的大小不能随「取多少块」变化 | $\pi_I = (S_I,\Lambda_I)$，永远两个群元素 |
| **验证常量** | 验证代价不能随**文件长度**增长 | 与 $n$ 无关，只与被打开的块数 $\lvert I\rvert$ 线性 |
...
验证时要拿它去算 $\Lambda_i^{e_i}\cdot S_i^{v_i} = C$，所以 $(S_1,\dots,S_n)$ 必须放进公开参数 ——
参数就变成了 $O(n)$。

**② 想要常数证据，就不能给每个位置单独发证明。**
逐位置证明的总大小是 $O(\lvert I\rvert)$。

**③ 想要能验证，就绕不开「求根」。**
证据天然是「某个群元素的 $\lvert I\rvert$ 次根」。而群是**隐藏阶**的
（模数 $N=pq$ 的分解在生成后被丢弃），求根正是 RSA 假设保护的困难问题。

**本方案对 ① 的解法**：公开参数里只放一个累加器

$$U_n = g^{e_{[n]}}, \qquad e_{[n]} = \prod_{i=1}^{n} e_i$$

让证明方把 $S_I = g^{e_{[n]}/e_I}$ 连同打开一起交出来，
验证方用**一次模幂**

$$S_I^{\,e_I} \stackrel{?}{=} U_n$$

就把它锁死成唯一可能的值。于是公开参数从 $O(n)$ 降到 $O(1)$。

**对 ③ 的解法**：注意 $e_j \cdot e_I \mid e_{[n]}$ 当且仅当 $j \notin I$。
所以只要 $j$ 不在被打开的下标里，
...
**渐进聚合**是本方案相对普通 SVC 的核心增量：合并结果 $\pi_K$ 仍然是一份合法的子向量打开证明，
...
> **$\pi_I$ 就是「去掉 $I$ 之后那个向量的摘要」。**

定义向量 $\mathbf{v}$ 的摘要

$$d(\mathbf{v}) = (S, \Lambda), \qquad
S = g^{e_{[n]}}, \quad
\Lambda = \prod_{i \in [n]} \left(S^{1/e_i}\right)^{v_i}$$

把这个定义里的 $[n]$ 换成 $[n]\setminus I$，得到的正是 $\pi_I$。于是：

| 操作 | 统一视角 | 代码 |
|---|---|---|
| **承诺** | $d(\mathbf{v}\setminus\varnothing) = (U_n, C)$ | `digest_of(crs_n, vals)` |
| **打开** | $d(\mathbf{v}\setminus I) = (S_I,\Lambda_I)$ | `digest_of(crs_n, vals, I)` |
| **拆分** | 把 $I\setminus K$ 逐个「加回」 | `disagg` |
| **验证** | 把 $I$ 全加回去，看是否变回 $d(\mathbf{v})$ | `verify` |
| **追加位置** | 对 $d(\mathbf{v})$ 加回一个新位置 | `update_append` |

「加回」只有一个公式，代价 $O(\ell)$：

$$(S,\ \Lambda) \ \longleftarrow\ \bigl(S^{e_i},\ \Lambda^{e_i}\cdot S^{v_i}\bigr)
\qquad\text{（}S^{v_i}\text{ 用更新前的 }S\text{）}$$

### 这个抽象直接带来了 150 倍加速

「验证 = 把 $I$ 逐个加回去」意味着每步的指数只有 $\ell+1$ 位；
而论文写法「先算 $S_i = S_I^{e_{I\setminus\{i\}}}$ 再乘起来」里，
每次重构的指数有 $(\lvert I\rvert-1)(\ell+1)$ 位，$\lvert I\rvert$ 次就是**平方级**。

| $\lvert I\rvert$ | 旧写法 | 新写法 | 加速 |
|---|---|---|---|
...
（$n=1024$、$\lvert N\rvert=1024$、$\ell=32$ 实测）
...
$$\prod_{j\in\text{inside}} \left(g^{\,e_{\text{inside}}/e_j}\right)^{v_j}$$
...
| 位置 | 朴素做法 | 本实现 |
|---|---|---|
| 一次算出所有 $S_i$ | $n$ 次「指数 $(n-1)(\ell+1)$ 位」的模幂 | `batch_root_factor`，$O(n\log n)$ |
| $e_{[n]}=\prod e_i$ | 顺序连乘 $O(n^2)$ | `product_tree` 平衡树，$O(n\log n)$ |
| $\Lambda_I$ 的 $1/e_I$ | 每个 $j\notin I$ 单独模幂 | `weighted_root_product`，一次分治 + 小指数加权 |

实测（$n=1024$、$\ell=32$）：分治比朴素快 **69 倍**；$n$ 再大差距继续拉大。

### 5.3 聚合为什么必须「除交叉项」

$$\rho_I = \frac{\Lambda_I}{\prod_{j\in J}\varphi_j^{v_j}}, \qquad
\sigma_J = \frac{\Lambda_J}{\prod_{i\in I}\psi_i^{v_i}}, \qquad
\Lambda_{I\cup J} = \text{ShamirTrick}(\rho_I, \sigma_J, e_I, e_J)$$

不做这一步直接相乘会重复计数：$\Lambda_I$ 里属于 $J$ 的那部分是 $1/e_I$ 次方，
而 $\Lambda_{I\cup J}$ 里应该是 $1/(e_I e_J)$ 次方，次数不匹配。
...
节点**不重新计算** $\Lambda_Q$，而是用 **disaggregation** 从手里的 $\pi_I$ 拆出来：
...
论文 §7 的正确性证明给了一个判据：
本地视图 $(pp,\delta,n,st,I,F_I)$ 合法 $\iff$ $st_1^{a_I}=\delta_1 \wedge st_2^{b_I}=\delta_2$。
...
| op | 摘要变化 | 节点状态变化 | 代价 |
|---|---|---|---|
| **`mod`** 改值 | $C' = C\cdot\prod_{i\in K}S_i^{\Delta_i}$，$U'=U$ | 只对 $i\in K\setminus I$ 修正 $\Lambda_I$ | $O(\lvert K\rvert\ell)$ |
| **`add`** 追加 | $(U',C')$ = 对 $(U,C)$ 顺序 `add_back` | $S'_I=S_I^{e_K}$，$\Lambda'_I=\Lambda_I^{e_K}\prod_{j\in K}(\cdot)^{v_j}$ | $O(\lvert K\rvert\ell)$ |
| **`del`** 删末尾 | $\delta' = \pi_K = d(\mathbf{v}\setminus K)$ | $K\subseteq I$ 时**一个字都不用改**；$I\cap K=\varnothing$ 时做一次 `agg` | $O(1)$ / $O(\lvert I\rvert\ell)$ |

几个漂亮的点：

- **`mod` 里，被改的位置若在节点自己的 $I$ 里，它的 $\Lambda_I$ 完全不变** ——
  因为 $\Lambda_I$ 只含 $j\notin I$ 的值。修正项里需要的 $S_I^{1/e_i}$
  恰好就是 $\text{ShamirTrick}(S_I,S_i,e_I,e_i)$。
- **`add` 之后，新位置的证明就是旧摘要本身**：$\pi_K^{\text{new}} = d_{\text{new}}(\mathbf{v}'\setminus K) = (U,C)$
  —— 因为「除 $K$ 之外的其余部分」正是旧文件。所以不用算任何东西。
- **`del` 的新摘要直接就是拆出来的 $\pi_K$**，与论文 $\delta' \leftarrow ((\Gamma_K,\Delta_K), n')$ 一致。
...
客户端手里只有 $\delta=((U,C),n)$。它要确认「服务器给的内容确实是当初承诺的那一份」。

1. **$S_I$ 锁死**：$S_I^{e_I}=U$。不知道群阶就求不出别的 $e_I$ 次根，
   所以 $S_I$ 只能是 $g^{e_{[n]}/e_I}$。
2. **值锁死**：$C = \Lambda_I^{e_I}\cdot\prod_{i\in I}S_i^{y_i}$。
   想换一个 $y_i$ 就得同时改 $\Lambda_I$，而 $\Lambda_I^{e_I}$ 的变化会被 $S_i^{y_i}$ 的变化抵消 ——
   除非能开出新的 $e_I$ 次根。
...
第二列值得单独说：**聚合算法自带净能力**。`VC.Agg` 在合并 $\Lambda$ 之前会用
...
| `Specialize` | **一次**指数有 $n(\ell+1)$ 位的模幂 | $n=2^{16},\ell{+}1{=}129$ → 指数 840 万位 → 数十秒 |
| `Commit` | 一次分治 + $n$ 次小指数模幂 | |
| `Open` | 一次分治 | |
| **`Verify`** | **$O(\ell\lvert I\rvert)$，与 $n$ 无关** | 实测 ~550 μs/元素，严格线性 |
| `Agg` | 2 次 Shamir + 2 次分治 | 与**被合并的块数**成正比，与 $n$ 无关 |
| `Disagg` | $O(\ell\lvert I\setminus K\rvert)$ | |
| `DisaggAll(n,B)` | $O(n\log(n/B))$，递归二分 | $n=1024,B=8$ → 128 块约 4.5 s |

**实测「验证与 $n$ 无关」**（$n$ 增长 16 倍，$\lvert I\rvert$ 固定为 8）：
...
$\lvert C\rvert$ 稳定在 1021–1024 位、$\lvert S_I\rvert$ 稳定在 1022–1024 位（都约等于 $\lvert N\rvert$）。
...
**① 聚合不能只验「能不能通过」。**
一个把 $\Lambda$ 算错但在别处凑巧抵消的实现，也能让「聚合后验证通过」成立。
...
完整逐条对照见 [`docs/与论文对照.md`](./与论文对照.md)。这里只列**未实现**的部分与原因。

### ✅ 已实现

- §5.2 全部算法：`Setup` / `Specialize` / `Com` / `Open` / `Ver` / `Disagg` / `Agg`
- 清单 #1~#26（含可选加速 `disagg_one_to_many` / `agg_many_to_one`）
- §8.2 主流程：`Bootstrap` / `AddStorage` / `RmvStorage` / `Retrieve` / `VerRetrieve` / `AggregateCertificates`
- §8.2 更新：`mod` / `add` / `del` 三种操作
...
1. **`shamir_trick` 的同源自检不能省**。它不只是「防错」，而是聚合算法自带的净能力：
   攻击场景下 $\rho_I$ 与 $\sigma_J$ 不同源，函数返回 $\bot$，连假证明都拼不出来。
2. **`Λ_∅ = C` 而不是 1**（空乘积 $e_\varnothing=1$）。$I=[n]$ 时 $\Lambda_I=1$ 才对。
   我自己一开始写错了，被自己的测试纠正。
...
- **证明就是摘要**：$\pi_I = d(\mathbf{v}\setminus I)$，承诺是 $I=\varnothing$ 的特例。
- **加回**：$(S,\Lambda)\leftarrow(S^{e_i},\ \Lambda^{e_i}S^{v_i})$ ——
  拆分、验证、追加三件事都只是把它循环几次。
- **$U_n$ 是那个「常数参数的秘密」**：公开参数里只放它，
  $S_I^{e_I}=U_n$ 一次模幂就把见证锁死了。
- **$1/e_I$ 能公开算，是因为 $j\notin I$ 时 $e_j\cdot e_I\mid e_{[n]}$** ——
  指数是整数，不用求根。
