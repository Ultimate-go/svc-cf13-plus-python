...
> 论文 §8.2 的 `PushUpdate`/`ApplyUpdate` 正文里用的是**另一套 SVC 构造**的原语
> （`Γ/∆`、`PartndPrimeProd`），而 §8.2 的摘要却是 §5.2 的形式。
> 也就是说更新算法正文的记号没与 §5.2 对齐，逐字转写做不到。
...
页面第 1 节既可以**上传文件**（按原始字节切块，二进制安全），
也可以直接在文本框里输入内容；不选文件时用文本框的 UTF-8 内容。
...
论文 §5 有**两个并列的 SVC 构造**，§8 相应地也有**两个并列的 VDS**。
本项目只做第二个（单生成元、承诺是一个群元素的那套）。
另一套（双生成元的「阴阳」方案 + 建立在它之上的 VDS，以及 §6 的知识论证族）
已经按范围要求移除 —— 它多出来的能力是「从一个已存文件派生新文件」，
而本方案**没有**这个算法，原因见下文「坑」第 7 条。

### ❌ 仍然没做的

| 未实现 | 原因 |
|---|---|
| 附录 C 的 `PoKChange` / `PoKAdd` / `PoKDelete` | 三种更新的**零知识**论证，建在另一个 SVC 构造上，不在本项目范围内。本实现的 §8.2 更新走「发布 `Υ_∆` + `VC.Ver` 验证」，功能上已是完整链路；只有「更新内容本身敏感」时才必需它们 |
| 并行化 | 素数生成与大批量模幂目前单线程。加速要用 `multiprocessing` 而非线程 —— CPython 的模幂计算期间**持有 GIL** |
| 附录 A / B / D.2 / E / F | A 是 `PoProd` 的低效写法（论文自己说更慢，属于另一个 SVC 构造）、B 是 [BBF19] 的预处理分析、D.2 是另一套 VDS 的 PDP（附录 D.1 的通用 PoR 已实现）、E 是强安全变体（验证时间线性于文件长度）、F 是与 BBF19 的实测对比 |
...
6. **`mod` 部分相交时抬的指数是 `a'_{L̄}` 而不是 `a'_K`**。
   `I ∩ K = L ∉ {K, ∅}` 时只有 `K` 落在 `I` 之外的那半个 `L̄` 会改变 `Λ_I`，
   抬整个 `a'_K` 会把 `I` 内部那份重复计入 —— 算出的状态通不过自检。
   这是 `tests/test_updates.py` 抳出来的。
7. **本方案拿不到「派生新文件」的能力，这是结构性的，不是没做**。
   那个能力靠一份子向量知识论证，而它需要「承诺是一对累加器」；
   本方案把承诺压成单个群元素（这正是它参数更省的原因），
   于是那套代数结构不存在。两个占位方法
   （`StorageNode.create_from` / `ClientNode.get_create`）会抛
